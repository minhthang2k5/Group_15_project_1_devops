...
class ProductServiceTest {
    ...
    @Test
    void testHandleProductVariationListFallback_shouldRethrowException() {
        RuntimeException exception = new RuntimeException("Service unavailable");

        assertThrows(Throwable.class, () -> productService.handleProductVariationListFallback(exception));
    }

    @Test
    void testHandleProductInfomationFallback_shouldRethrowException() {
        RuntimeException exception = new RuntimeException("Service unavailable");

        assertThrows(Throwable.class, () -> productService.handleProductInfomationFallback(exception));
    }

    @Test
    void testSubtractProductStockQuantity_withOrderItems_shouldNoException() {
        OrderItemVm item = new OrderItemVm(
            1L, 101L, "Product A", 2, new java.math.BigDecimal("29.99"),
            "Note", new java.math.BigDecimal("5.00"), new java.math.BigDecimal("2.00"),
            new java.math.BigDecimal("8.00"), 1001L
        );
        OrderVm orderVm = mock(OrderVm.class);
        when(orderVm.orderItemVms()).thenReturn(Set.of(item));
        ...
        org.junit.jupiter.api.Assertions.assertDoesNotThrow(() -> productService.subtractProductStockQuantity(orderVm));
    }
}
