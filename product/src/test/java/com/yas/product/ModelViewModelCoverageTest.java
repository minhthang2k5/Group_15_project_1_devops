...
public class ModelViewModelCoverageTest {
    ...
    void testModelsAndViewModels() {
        String srcPath = "src/main/java/com/yas/product";
        File srcDir = new File(srcPath);
        if (!srcDir.exists()) {
            srcDir = new File("product/src/main/java/com/yas/product");
        }
        
        if (srcDir.exists()) {
            List<Class<?>> classes = new ArrayList<>();
            scanDir(srcDir, "com.yas.product", classes);
    ...
    private void scanDir(File dir, String packageName, List<Class<?>> classes) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                ...
                try {
                    if (!className.contains("ProductApplication") &&
